...
const Cart = () => {
    ...
    const totoalPrice = items.reduce((total, item) => total + item.price * item.quantity, 0).toFixed(2)

    // vat

    const vat = (+totoalPrice * 0.15).toFixed(2)

    const totalPriceWithVat = (+totoalPrice + +vat).toFixed(2)
    ...
    const removeItemHnadler = (id: number) => {
        ...
        <div className='mt-8 min-h-[60vh]'>
            {items.length == 0 && (
                <div className='flex items-center w-full h-[80vh] flex-col justify-center'>
                    <Image src="/images/cart.svg" alt='empty' width={400} height={400} className='object-cover mx-auto' />
            ...
            {items.length > 0 && (
                ...
                <div className='md:w-4/5 w-[95%] mx-auto grid grid-cols-1 xl:grid-cols-6 gap-12'>
                    <div className='rounded-lg shadow-md overflow-hidden xl:col-span-4'>
                        <h1 className='p-4 text-xl sm:text-2xl font-bold text-white bg-blue-700'>Your cart ({totalQuantity} Items)</h1>
                        {items.map((item) => {
                                ...
                                <div className='flex pb-6 mt-2 p-5 border-b-[1.5px] border-opacity-25 border-gray-700 items-center space-x-10'>
                                    ...
                                    <div>
                                        ...
                                        <div className='flex items-center mt-4 space-x-2'>
                                            ...
                                            <Button onClick={()=>{removeItemHnadler(item.id)}} variant={"destructive"}>Remove</Button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className='xl:col-span-2'>
                        <div className='bg-indigo-900 sticky top-[25vh] p-6 rounded-lg'>
                            ...
                            )}
                            {
                                ...
                                (
                                    //paypel button
                                    <Button className='w-full bg-orange-500'>Paypel</Button>
                                )
                            }
                        </div>
                    </div>
